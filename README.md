# 🔐 ASP.NET Web API с JWT аутентификацией

Полный пример защищенного ASP.NET Web API с JWT аутентификацией, WPF и веб-клиентами для локальной сети предприятия.

## 🚀 Особенности

- **ASP.NET Web API 10.0** с JWT аутентификацией
- **WPF клиент** для десктоп приложений
- **Веб-клиент** (HTML/CSS/JS) для браузеров
- **MemoryCache** для управления сессиями
- **Ролевая модель** (User/Admin)
- **Мгновенный logout** с инвалидацией токенов
- **Защита от прямого доступа** (только через приложение)

## 📁 Структура проекта
ASP.NET-API-Example/
├── ASP.NET-API/ # Backend API (C#)
├── WPF-Client/ # WPF desktop client (C#)
├── Web-Client/ # Web client (HTML/JS)
└── Tests/ # PowerShell тесты

## 🛠️ Быстрый старт

### 1. Запуск API
```powershell
cd ASP.NET-API
dotnet run

API будет доступен по: http://localhost:5261 (либо другая ссылка,
в зависимости от сертификаиа и выбранного порта)

Запустите PowerShell тесты:
.\Tests\api-tests.ps1
.\Tests\security-tests.ps1

📡 API Эндпоинты
🔓 Публичные

    GET /api/public/info - Информация о API

    GET /api/public/health - Проверка здоровья

🔐 Аутентификация

    POST /api/auth/login - Получить JWT токен

    POST /api/auth/logout - Выйти (инвалидация токена)

    GET /api/auth/validate - Проверить валидность токена

🛡️ Защищенные данные

    GET /api/data/user-info - Данные пользователя

    GET /api/data/admin-data - Админ данные (только Admin)

    GET /api/data/stats - Статистика

🧪 Тестирование

PowerShell тесты:
# Полный тест API
.\Tests\api-tests.ps1

# Тест безопасности
.\Tests\security-tests.ps1

# Нагрузочный тест
.\Tests\load-test.ps1

Ручное тестирование:
# 1. Получить токен
$body = '{"username":"user1","password":"password1"}' | ConvertFrom-Json | ConvertTo-Json
$resp = Invoke-RestMethod http://localhost:5261/api/auth/login -Method Post -Body $body

# 2. Использовать токен
$headers = @{"Authorization"="Bearer $($resp.Token)";"User-Agent"="Test/1.0"}
Invoke-RestMethod http://localhost:5261/api/data/user-info -Headers $headers

# 3. Выйти
Invoke-RestMethod http://localhost:5261/api/auth/logout -Method Post -Headers $headers

🖥️ Клиенты
WPF клиент

    Полноценное десктоп приложение

    Сохранение сессии между запусками

    Проверка валидности токена

Веб-клиент

    Простой HTML/JS интерфейс

    Работает в любом современном браузере

    Логирование всех запросов

    Тесты безопасности

⚙️ Технологии

    Backend: ASP.NET 10.0, JWT Bearer, MemoryCache

    WPF: .NET 10.0, System.Text.Json

    Web: Vanilla JavaScript, Fetch API

    Тесты: PowerShell, Invoke-RestMethod

🔒 Безопасность

    JWT токены с подписью SHA256

    Проверка токенов в кэше при каждом запросе

    Защита от CSRF (CORS политики)

    Проверка User-Agent для защиты от прямого доступа

    Ролевая модель доступа
